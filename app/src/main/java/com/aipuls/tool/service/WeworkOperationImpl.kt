package com.aipuls.tool.service
...
import com.aipuls.tool.model.ExecCallbackBean
import com.aipuls.tool.model.operation.SelectResult
import com.aipuls.tool.utils.*
import java.io.File
...
//        uploadCommandResult(message, ExecCallbackBean.SUCCESS, "", startTime, successList, failList)
        ...
        for (title in LinkedHashSet(titleList)) {
            if (WeworkRoomUtil.intoRoom(title) || WeworkRoomUtil.intoRoom(title, fastIn = false)) {
                if (WeworkTextUtil.longClickMessageItem(
                        ...
                        AccessibilityUtil.findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.ListView),
                        textType,
                        receivedName,
                        originalContent,
                        "回复", "引用"
    ...
    fun pushMicroDiskImage(
        ...
        if (node != null) {
            ...
            if (buttonList.size >= 4) {
                ...
                val imageViewList = AccessibilityUtil.findAllByClazz(getRoot(), com.aipuls.tool.utils.Views.ImageView)
    ...
    private fun subSendMultiMessage(
        ...
        if (WeworkRoomUtil.intoRoom(groupName) || WeworkRoomUtil.intoRoom(groupName, fastIn = false)) {
            ...
            //聊天消息列表 1ListView 0RecycleView xViewGroup
            val list = AccessibilityUtil.findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.ListView)
            if (list != null) {
                    ...
                    if (item != null && item.childCount > 0) {
                        ...
                        if (start) {
                            if (Constant.version >= 40108) {
                                if (AccessibilityUtil.findOnceByClazz(item, com.aipuls.tool.utils.Views.CheckBox)?.isChecked == first) {
            ...
            val bottomList = AccessibilityUtil.findOnceByClazz(getRoot(), com.aipuls.tool.utils.Views.ViewGroup, minChildCount = 4)
    ...
    fun addFriendByPhone(
        ...
        //手机号添加好友
        goHome()
        val list = AccessibilityUtil.findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.RecyclerView, com.aipuls.tool.utils.Views.ListView, com.aipuls.tool.utils.Views.ViewGroup)
        if (list != null) {
            val frontNode = AccessibilityUtil.findFrontNode(list)
            val textViewList = AccessibilityUtil.findAllOnceByClazz(frontNode, com.aipuls.tool.utils.Views.TextView)
            if (textViewList.size >= 2) {
                ...
                sleep(Constant.POP_WINDOW_INTERVAL)
                val list = AccessibilityUtil.findAllByClazz(getRoot(), com.aipuls.tool.utils.Views.ListView).lastOrNull()
                if (list != null) {
                            ...
                            if (bothUsedText != null && bothUsedText.contains("对方同时使用")) {
                                AccessibilityUtil.performClick(
                                    AccessibilityUtil.findOnceByClazz(
                                        ...
                                        com.aipuls.tool.utils.Views.ImageView
                                    )
                                )
            ...
            if (AccessibilityUtil.findTextAndClick(getRoot(), "查看全部群成员")) {
                ...
                if (list != null) {
                    ...
                    if (textViewList.size >= 2) {
                        ...
                        sleep(Constant.CHANGE_PAGE_INTERVAL)
                        //消息页搜索结果列表
                        val selectListView = AccessibilityUtil.findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.ListView)
                        ...
                        if (selectListView != null && matchSelect != null) {
                            for (i in 0 until selectListView.childCount) {
                                ...
                                if (searchResult?.parent != null && searchResult.parent.childCount < 3) {
                                    item.refresh()
                                    val imageView = AccessibilityUtil.findOneByClazz(item, com.aipuls.tool.utils.Views.ImageView, root = false)
    ...
    fun addNeedDeal(
        ...
        if (tvDiaryFlag != null && (tvDiaryFlag.parent?.childCount == 2 || tvDiaryFlag.parent?.childCount == 3)) {
            ...
            if (tvNeedDealFlag != null) {
                ...
                sleep(Constant.POP_WINDOW_INTERVAL)
                val list = AccessibilityUtil.findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.RecyclerView, com.aipuls.tool.utils.Views.ViewGroup, minChildCount = 2)
                if (list != null) {
                    val frontNode = AccessibilityUtil.findFrontNode(if (list.className == com.aipuls.tool.utils.Views.RecyclerView) list.parent else list)
                    val textViewList =
                        AccessibilityUtil.findAllOnceByClazz(frontNode, com.aipuls.tool.utils.Views.TextView)
                    if (textViewList.size >= 2) {
    ...
    fun switchCorp(message: WeworkMessageBean, objectName: String): Boolean {
        ...
        goHomeTab("消息")
        val firstTv = AccessibilityUtil.findAllByClazz(getRoot(), com.aipuls.tool.utils.Views.TextView)
            .firstOrNull { it.text == null }
        AccessibilityUtil.performClick(firstTv, retry = false)
        sleep(Constant.CHANGE_PAGE_INTERVAL)
        val listviewList = AccessibilityUtil.findAllOnceByClazz(getRoot(), com.aipuls.tool.utils.Views.RecyclerView, com.aipuls.tool.utils.Views.ListView, com.aipuls.tool.utils.Views.ViewGroup)
    ...
    fun deleteContact(message: WeworkMessageBean, friend: WeworkMessageBean.Friend): Boolean {
        ...
        if (friend.phone == null && friend.name != null) {
            if (getFriendInfo(friend.name) && AccessibilityExtraUtil.loadingPage("ContactDetailBaseContentActivity")) {
                val imageView = AccessibilityUtil.findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.ImageView)
        ...
        if (list != null) {
            ...
            if (textViewList.size >= 2) {
                ...
                for (select in LinkedHashSet(selectList)) {
                    ...
                    AccessibilityUtil.findTextInput(getRoot(), trimTitle)
                    sleep(Constant.CHANGE_PAGE_INTERVAL)
                    val selectListView = AccessibilityUtil.findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.ListView, com.aipuls.tool.utils.Views.RecyclerView, com.aipuls.tool.utils.Views.ViewGroup, minChildCount = 2)
                    val reverseRegexTitle = RegexHelper.reverseRegexTitle(trimTitle)
                    val regex1 = (if (Constant.friendRemarkStrict) "^$reverseRegexTitle" else "^(微信昵称:)|((企业)?邮箱:)?$reverseRegexTitle") +
                            (if (needTrim) ".*?" else Constant.suffixString)
                    val regex2 = ".*?\\($reverseRegexTitle\\)$"
    ...
    private fun groupRename(groupName: String?): Boolean {
        ...
        if (WeworkRoomUtil.intoGroupManager()) {
            ...
            if (textView.text?.toString()?.contains("微信用户创建") == true) {
                ...
                if (button?.className == com.aipuls.tool.utils.Views.ImageView) {
    ...
    private fun groupAddMember(
        ...
        if (WeworkRoomUtil.intoGroupManager()) {
            AccessibilityUtil.findOneByText(getRoot(), "全部群成员", "微信用户创建", timeout = Constant.CHANGE_PAGE_INTERVAL)
                ?: return selectResult
            val gridView = AccessibilityUtil.findOnceByClazz(getRoot(), com.aipuls.tool.utils.Views.GridView)
            if (gridView != null && gridView.childCount >= 2) {
                val tvEmptySize = AccessibilityUtil.findAllOnceByClazz(gridView, com.aipuls.tool.utils.Views.TextView)
                ...
                val recyclerViewList = AccessibilityUtil.findAllOnceByClazz(getRoot(), com.aipuls.tool.utils.Views.RecyclerView)
            ...
            if (list != null) {
                ...
                if (textViewList.size >= 2) {
                    ...
                    for (select in LinkedHashSet(selectList)) {
                        ...
                        AccessibilityUtil.findTextInput(getRoot(), trimTitle)
                        sleep(Constant.POP_WINDOW_INTERVAL)
                        val selectListView = AccessibilityUtil.findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.ListView, com.aipuls.tool.utils.Views.RecyclerView, com.aipuls.tool.utils.Views.ViewGroup, minChildCount = 2, firstChildClazz = com.aipuls.tool.utils.Views.TextView)
                        val reverseRegexTitle = RegexHelper.reverseRegexTitle(trimTitle)
                        val regex1 = (if (Constant.friendRemarkStrict) "^$reverseRegexTitle" else "^(微信昵称:)|((企业)?邮箱:)?$reverseRegexTitle") +
                                (if (needTrim) ".*?" else Constant.suffixString)
                        val regex2 = ".*?\\($reverseRegexTitle\\)$"
                        ...
                        if (selectListView != null && (matchSelect != null || reverseRegexTitle.matches(
                                Constant.regMail))) {
                            var flag = false
                            for (i in 0 until selectListView.childCount) {
                                val item = selectListView.getChild(i)
                                if ((matchSelect == null && item.className == com.aipuls.tool.utils.Views.RelativeLayout)
                        ...
                        val noResult = AccessibilityUtil.findOnceByText(getRoot(), "无搜索结果", "暂无搜索结果", exact = true) != null
                        val textView = AccessibilityUtil.findOnceByClazz(getRoot(), com.aipuls.tool.utils.Views.TextView)
            ...
            if (list != null) {
                ...
                if (textViewList.size >= 2) {
                    ...
                    for (select in LinkedHashSet(removeList)) {
                        ...
                        AccessibilityUtil.findTextInput(getRoot(), trimTitle)
                        sleep(Constant.POP_WINDOW_INTERVAL)
                        val selectListView = AccessibilityUtil.findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.ListView, com.aipuls.tool.utils.Views.RecyclerView, com.aipuls.tool.utils.Views.ViewGroup, minChildCount = 2, firstChildClazz = com.aipuls.tool.utils.Views.RelativeLayout)
                        val reverseRegexTitle = RegexHelper.reverseRegexTitle(trimTitle)
                        val regex1 = (if (Constant.friendRemarkStrict) "^$reverseRegexTitle" else "^(微信昵称:)|((企业)?邮箱:)?$reverseRegexTitle") +
                                (if (needTrim) ".*?" else Constant.suffixString)
                        val regex2 = ".*?\\($reverseRegexTitle\\)$"
    ...
    private fun sendChatMessage(text: String, at: String? = null, atList: List<String>? = null, reply: Boolean? = false, title: String): Boolean {
        ...
        if (!atList.isNullOrEmpty() && (roomType == WeworkMessageBean.ROOM_TYPE_INTERNAL_GROUP || roomType == WeworkMessageBean.ROOM_TYPE_EXTERNAL_GROUP)) {
            if (atList.count { it.startsWith("#regex#") } > 0 && WeworkRoomUtil.intoGroupManager()) {
                val groupInfo =
                    WeworkGetImpl.getGroupInfoDetail(saveAddress = false, saveMembers = true)
            ...
            LinkedHashSet(atList).forEachIndexed { index, at ->
                if (index == 0) {
                    ...
                    AccessibilityUtil.sendTextForEditText(Utils.getApp(),
                        AccessibilityUtil.findOnceByClazz(getRoot(), com.aipuls.tool.utils.Views.EditText), "@"
                    )
                }
...
//                WeworkLoopImpl.getChatMessageList(needInfer = false, titleList = titleList)
    ...
    private fun getFriendInfo(title: String): Boolean {
        goHomeTab("通讯录")
        val list = AccessibilityUtil.findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.ListView, com.aipuls.tool.utils.Views.RecyclerView)
        if (list != null) {
    ...
    private fun modifyFriendInfo(friend: WeworkMessageBean.Friend, addFriend: Boolean = true): Boolean {
        if (AccessibilityUtil.findOneByText(getRoot(), "标签", "电话", "描述", "设置备注和描述", exact = true) != null) {
            ...
            if (markTv != null && (friend.markName != null
                ...
                val etList =
                    AccessibilityUtil.findAllByClazz(getRoot(), com.aipuls.tool.utils.Views.EditText, minSize = 2)
                if (etList.size >= 4) {
                    //微信用户 备注/企业/电话/电话/描述
            ...
            val imageView = AccessibilityUtil.findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.ImageView)
            if (imageView != null) {
                val textViewList = AccessibilityUtil.findAllOnceByClazz(imageView.parent, com.aipuls.tool.utils.Views.TextView)
    ...
    fun setFriendTags(tagList: List<String>): Boolean {
        ...
        if (list != null && list.childCount > 0) {
            for (i in 0 until list.childCount) {
                ...
                if (child.className.equals(com.aipuls.tool.utils.Views.TextView) && child.text != null) {
    ...
    private fun getGroupQrcode(groupName: String, groupRemark: String?): Boolean {
        ...
        if (AccessibilityUtil.findOneByText(getRoot(), "全部群成员", "微信用户创建", timeout = Constant.CHANGE_PAGE_INTERVAL) != null ||
            ...
            val tvList = AccessibilityUtil.findAllOnceByClazz(getRoot(), com.aipuls.tool.utils.Views.TextView)
            ...
            while (currentTime - startTime <= Constant.CHANGE_PAGE_INTERVAL * 5) {
                AccessibilityUtil.findOnceByClazz(getRoot(), com.aipuls.tool.utils.Views.ProgressBar) ?: break

...
import com.aipuls.tool.utils.AccessibilityUtil
...
fun backPress() {
    ...
    val root = getRoot()
    val textView = AccessibilityUtil.findOnceByClazz(root, com.aipuls.tool.utils.Views.TextView)
    if (textView != null && textView.text.isNullOrBlank() && AccessibilityUtil.performClick(textView, retry = false)) {
        ...
        if (AccessibilityUtil.findOnceByText(root, "移出成员") != null) {
            val list = AccessibilityUtil.findOneByClazz(root, com.aipuls.tool.utils.Views.RecyclerView, com.aipuls.tool.utils.Views.ListView)
            if (list != null) {
        ...
        val ivButton = AccessibilityUtil.findOnceByClazz(root, com.aipuls.tool.utils.Views.ImageView)
        if (ivButton != null && ivButton.isClickable && AccessibilityUtil.findFrontNode(ivButton) == null) {
            ...
            val button = AccessibilityUtil.findOnceByClazz(root, com.aipuls.tool.utils.Views.Button)
                ...
                if (confirm != null) {
                    ...
                    if (stayButton != null) {
                        ...
                        if (WeworkController.weworkService.currentClass == clazz) {
                            val firstEmptyTextView = AccessibilityUtil.findAllByClazz(getRoot(), com.aipuls.tool.utils.Views.TextView).firstOrNull { it.text.isNullOrEmpty() }
 ...
 * 上传执行指令结果
 */
fun uploadCommandResult(message: WeworkMessageBean, errorCode: Int, errorReason: String, startTime: Long, successList: List<String> = listOf(), failList: List<String> = listOf(), metaJson: String? = null) {
    ...
    WeworkController.weworkService.webSocketManager.send(
        WeworkMessageListBean(
            ExecCallbackBean(GsonUtils.toJson(message), errorCode, errorReason, startTime, (System.currentTimeMillis() - startTime) / 1000.0, successList, failList, metaJson),

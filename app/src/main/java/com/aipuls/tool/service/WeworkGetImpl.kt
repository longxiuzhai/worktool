...
object WeworkGetImpl {
            ...
            val onScrollListener = object : AccessibilityUtil.OnScrollListener() {
                override fun onScroll(): Boolean {
                    ...
                    for (i in 0 until list.childCount) {
                        ...
                        if (item.className != com.aipuls.tool.utils.Views.RelativeLayout) {
                            continue
                        }
                        val name = AccessibilityUtil.findOneByClazz(item, com.aipuls.tool.utils.Views.TextView)?.text?.toString()
                            ?: continue
        ...
        if (AccessibilityUtil.findTextAndClick(getRoot(), "群聊", exact = true)) {
            ...
            val onScrollListener = object : AccessibilityUtil.OnScrollListener() {
                override fun onScroll(): Boolean {
                    ...
                    list.refresh()
                    AccessibilityUtil.findAllOnceByClazz(list, com.aipuls.tool.utils.Views.TextView).map {
    ...
    fun getMyInfo(message: WeworkMessageBean): Boolean {
        if (!goHomeTab("我")) {
            ...
            if (list != null) {
                ...
                for (i in 0 until list.childCount) {
                    val item = list.getChild(i)
                    val tvList = AccessibilityUtil.findAllOnceByClazz(item, com.aipuls.tool.utils.Views.TextView)
            ...
            val newFirstTv = AccessibilityUtil.findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.TextView)
            ...
            if (nickname != null) {
                ...
                if (AccessibilityUtil.performClick(newFirstTv)) {
                    sleep(Constant.CHANGE_PAGE_INTERVAL)
                    val list = AccessibilityUtil.findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.RecyclerView, com.aipuls.tool.utils.Views.ListView, com.aipuls.tool.utils.Views.ViewGroup, minChildCount = 2)
                    if (list != null) {
                        val myInfoLayout = list.getChild(0)
                        val tvList = AccessibilityUtil.findAllByClazz(myInfoLayout, com.aipuls.tool.utils.Views.TextView)
                            .filter { it.text != null }
        ...
        AccessibilityUtil.performClick(AccessibilityUtil.findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.ImageView))
        sleep(Constant.CHANGE_PAGE_INTERVAL)
        val relativeLayoutList = AccessibilityUtil.findAllByClazz(getRoot(), com.aipuls.tool.utils.Views.RelativeLayout, minSize = 50)
        val myInfo = WeworkMessageBean.MyInfo()
        for (relativeLayout in relativeLayoutList.filter { it.childCount >= 2 }) {
            val textViewList = AccessibilityUtil.findAllOnceByClazz(relativeLayout, com.aipuls.tool.utils.Views.TextView)
    ...
    fun getGroupInfoDetail(saveAddress: Boolean = true, saveMembers: Boolean = false): WeworkMessageBean {
        ...
        if (tvManagerFlag?.text?.toString()?.contains("微信用户创建") == true) {
            ...
            val tvGroupName = AccessibilityUtil.findOnceByClazz(button, com.aipuls.tool.utils.Views.TextView)
        ...
        if (gridView != null && gridView.childCount >= 2) {
            ...
            val tvOwnerName = AccessibilityUtil.findOnceByClazz(gridView.getChild(0), com.aipuls.tool.utils.Views.TextView)
            ...
            if ((weworkMessageBean.groupNumber ?: 0) <= 8) {
                ...
                for (i in 0 until gridView.childCount) {
                    ...
                    val name = AccessibilityUtil.findOnceByClazz(item, com.aipuls.tool.utils.Views.TextView)?.text?.toString()
                        ?: continue
                    set.add(name)
                }
            ...
            if (recyclerViewList.size >= 2 && recyclerViewList[1].childCount >= 2) {
                ...
                val tvOwnerName = AccessibilityUtil.findOnceByClazz(rvList.getChild(0), com.aipuls.tool.utils.Views.TextView)
                if (tvOwnerName != null && tvOwnerName.text != null) {
                    LogUtils.d("群主: " + tvOwnerName.text)
                    weworkMessageBean.groupOwner = tvOwnerName.text.toString()
                }
        ...
        val tvRemark = AccessibilityUtil.findOnceByClazz(AccessibilityUtil.findBackNode(tvRemarkFlag), com.aipuls.tool.utils.Views.TextView)
        ...
        if (saveMembers && weworkMessageBean.nameList.isNullOrEmpty()) {
            if (AccessibilityUtil.findTextAndClick(getRoot(), "查看全部群成员", exact = true, timeout = 0)) {
                val userList = AccessibilityUtil.findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.ListView)
                if (userList != null) {
                    val set = linkedSetOf<String>()
                    val onScrollListener = object : AccessibilityUtil.OnScrollListener() {
                        override fun onScroll(): Boolean {
            ...
            val onScrollListener = object : AccessibilityUtil.OnScrollListener() {
                override fun onScroll(): Boolean {
                    ...
                    for (i in 0 until list.childCount) {
                        ...
                        val tvList = AccessibilityUtil.findAllOnceByClazz(item, com.aipuls.tool.utils.Views.TextView).mapNotNull { it.text }.filter { !it.startsWith("＠") }
    ...
    private fun getRealName(nickname: String) {
        if (AccessibilityExtraUtil.loadingPage("SettingMineInfoActivity")) {
            ...
            val tvExternal = AccessibilityUtil.findAllOnceByClazz(external?.parent, com.aipuls.tool.utils.Views.TextView).firstOrNull { it.text != null && it.text.toString().contains("＠") }
            ...
            if (AccessibilityUtil.findTextAndClick(getRoot(), "姓名", exact = true)) {
                ...
                if (realNameFlag != null) {
                    ...
                    val realName = AccessibilityUtil.findOnceByClazz(AccessibilityUtil.findBackNode(realNameFlag, 1), com.aipuls.tool.utils.Views.TextView)
                    if (notRealName != null) {

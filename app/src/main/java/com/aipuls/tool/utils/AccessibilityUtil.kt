...
object AccessibilityUtil {
    ...
    fun scrollToTop(
        ...
        while (index++ < maxRetry) {
            val scrollBefore = findAllOnceByClazz(getRoot(), com.aipuls.tool.utils.Views.TextView)
            performScrollUp(nodeInfo, scrollNodeIndex)
            if (scrollBefore == findAllOnceByClazz(getRoot(), com.aipuls.tool.utils.Views.TextView)) {
                LogUtils.d("已经滚动到顶部")
                if (textChanged) {
                    return true
                } else {

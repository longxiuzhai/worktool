package com.aipuls.tool.utils

import android.view.accessibility.AccessibilityNodeInfo
import com.aipuls.tool.utils.AccessibilityUtil.findOneByClazz
import com.aipuls.tool.utils.AccessibilityUtil.findFrontNode
...
import com.aipuls.tool.service.WeworkGetImpl
import com.aipuls.tool.service.backPress
import com.aipuls.tool.service.getRoot
import com.aipuls.tool.service.goHome
import com.aipuls.tool.service.sleep
import com.aipuls.tool.service.*
import com.aipuls.tool.utils.AccessibilityUtil.findAllOnceByClazz
...
object WeworkRoomUtil {
    ...
    fun getRoomTitle(print: Boolean = true, noCut: Boolean = false): ArrayList<String> {
        ...
        val list = AccessibilityUtil.findOnceByClazz(getRoot(), com.aipuls.tool.utils.Views.ListView)
        if (list != null) {
            val frontNode = findFrontNode(list.parent?.parent)
            val textViewList = findAllOnceByClazz(frontNode, com.aipuls.tool.utils.Views.TextView)
    ...
    fun intoRoom(title: String, fastIn: Boolean = true): Boolean {
        ...
        goHome()
        val list = findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.RecyclerView, com.aipuls.tool.utils.Views.ListView, com.aipuls.tool.utils.Views.ViewGroup)
        if (fastIn) {
            if (list != null && list.childCount >= 2) {
                for (i in 0 until list.childCount) {
                    val item = list.getChild(i)
                    val tvList = findAllOnceByClazz(item, com.aipuls.tool.utils.Views.TextView).mapNotNull { it.text }
        ...
        if (list != null) {
            ...
            if (textViewList.size >= 2) {
                ...
                //消息页搜索结果列表
                val selectListView = findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.ListView)
                ...
                if (searchResult.isNotEmpty()) {
                    ...
                    if (searchItem != null) {
                        ...
                        val text = AccessibilityUtil.findAllOnceByClazz(searchItem.parent, com.aipuls.tool.utils.Views.TextView).firstOrNull { it.text != null && !it.text.isNullOrBlank() }
    ...
    fun intoGroupManager(): Boolean {
        ...
        //群详情列表
        val list = findOneByClazz(getRoot(), com.aipuls.tool.utils.Views.ListView)
        if (list != null) {
    ...
    fun getFriendName(): ArrayList<String> {
        ...
        if (intoFriendDetail()) {
            ...
            if (gridView != null && gridView.childCount >= 2) {
                ...
                val tvList = findAllOnceByClazz(gridView.getChild(0), com.aipuls.tool.utils.Views.TextView)
                ...
                if (recyclerViewList.size >= 2 && recyclerViewList[1].childCount >= 2) {
                    ...
                    val tvList = findAllOnceByClazz(rvList.getChild(0), com.aipuls.tool.utils.Views.TextView)
                    for (textView in tvList) {
                        if (textView.text != null) {
                            titleList.add(textView.text.toString())
                        }
    ...
    private fun isExternalGroup(): Boolean {
        //聊天消息列表 1ListView 0RecycleView xViewGroup
        val listView = AccessibilityUtil.findOnceByClazz(getRoot(), com.aipuls.tool.utils.Views.ListView, limitDepth = null, depth = 0)
        if (listView != null) {
    ...
    private fun isSingleChat(): Boolean {
        //聊天消息列表 1ListView 0RecycleView xViewGroup
        val list = AccessibilityUtil.findOnceByClazz(getRoot(), com.aipuls.tool.utils.Views.ListView)
        val editText = AccessibilityUtil.findOnceByClazz(getRoot(), com.aipuls.tool.utils.Views.EditText)

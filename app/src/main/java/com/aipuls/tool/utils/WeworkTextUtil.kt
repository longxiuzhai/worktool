...
import com.aipuls.tool.utils.AccessibilityUtil.findAllByClazz
...
带回复引用文本 1tv 2iv (引用发言人、引用发言内容、本次消息内容)
    ...
    fun getTextType(node: AccessibilityNodeInfo?, isGroup: Boolean = true): Int {
        ...
        while (System.currentTimeMillis() - startTime <= Constant.LONG_INTERVAL) {
            node.refresh()
            AccessibilityUtil.findOnceByClazz(node, com.aipuls.tool.utils.Views.ProgressBar) ?: break
            LogUtils.e("发现加载项 等待加载完成...")
            sleep(Constant.POP_WINDOW_INTERVAL / 5)
        }
        val tvList = findAllOnceByClazz(node, com.aipuls.tool.utils.Views.TextView)
        val tvCount = tvList.size
        val ivCount = findAllOnceByClazz(node, com.aipuls.tool.utils.Views.ImageView).size
    ...
    fun getTextTypeFromItem(node: AccessibilityNodeInfo?, isGroup: Boolean = true): Int {
        ...
        if (relativeLayoutItem != null && relativeLayoutItem.childCount >= 2) {
            if (com.aipuls.tool.utils.Views.ImageView.equals(relativeLayoutItem.getChild(relativeLayoutItem.childCount - 2).className)) {
            ...
            } else if (com.aipuls.tool.utils.Views.ImageView.equals(relativeLayoutItem.getChild(relativeLayoutItem.childCount - 1).className)) {
    ...
    fun getNameList(item: AccessibilityNodeInfo): List<String> {
        ...
        val node = AccessibilityUtil.findOnceByClazz(item, com.aipuls.tool.utils.Views.ViewGroup)
        if (node != null) {
            val textViewList = findAllOnceByClazz(node, com.aipuls.tool.utils.Views.TextView)
    ...
    private fun longClickMessageItem(item: AccessibilityNodeInfo, roomType: Int, vararg key: String): Boolean {
        ...
        val optionRvList = findAllByClazz(getRoot(), com.aipuls.tool.utils.Views.RecyclerView, com.aipuls.tool.utils.Views.ViewGroup)
    ...
    private fun getMessageListNode(item: AccessibilityNodeInfo, roomType: Int): AccessibilityNodeInfo? {
        if (roomType in arrayOf(WeworkMessageBean.ROOM_TYPE_INTERNAL_CONTACT, WeworkMessageBean.ROOM_TYPE_EXTERNAL_CONTACT)) {
            val node = AccessibilityUtil.findOnceByClazz(item, com.aipuls.tool.utils.Views.ImageView)
            if (node != null) {
    ...
    private fun getMyMessageListNode(item: AccessibilityNodeInfo): AccessibilityNodeInfo? {
        val node = AccessibilityUtil.findAllOnceByClazz(item, com.aipuls.tool.utils.Views.ImageView).lastOrNull()
